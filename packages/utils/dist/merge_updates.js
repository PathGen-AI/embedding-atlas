 ...
 * // result: { b: { c: 4, d: 3 } }
 */
export function mergeUpdates(value, updates) {
...
export function applyUpdatesIfNeeded(value, updates, mode, setter) {
    switch (mode) {
        case "merge": {
...
export function applyUpdatesForKeyIfNeeded(records, key, update, mode, setter) {
    switch (mode) {
        case "merge": {
            let r = mergeUpdates(records, { [key]: update });
            if (r !== undefined) {
                return setter(r);
            }
            else {
...
//# sourceMappingURL=merge_updates.js.map